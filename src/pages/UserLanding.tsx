...
const UserLanding = () => {
  ...
  return <div className="min-h-screen bg-gradient-subtle">
      ...
      <main className="container mx-auto px-4 pt-20 md:pt-24 pb-24">
        ...
        {/* Category Tabs with Service Pills - Carousel on Mobile */}
        <div className="mb-8 md:mb-12">
          <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="w-full">
                  ...
                  {categoryTabsData.map((category) => (
                    <CarouselItem key={category.id} className="pl-2 basis-1/4">
                      <TabsList className="w-full h-auto bg-transparent p-0">
                        <TabsTrigger
                          value={category.id}
                          className="w-full flex flex-col items-center gap-1 p-2 data-[state=active]:bg-primary/15 data-[state=active]:text-primary rounded-lg transition-all h-auto"
                        >
                          ...
                          <span className="text-[10px] font-medium text-center leading-tight">
                            {category.id === 'auto' ? 'Auto' : category.id === 'electrician' ? 'Luz' : category.id === 'plumbing' ? 'Agua' : category.name}
                          </span>
                        </TabsTrigger>
                      </TabsList>
                    </CarouselItem>
            ...
            {/* Desktop: Grid */}
            <TabsList className="hidden md:grid w-full h-auto bg-card/80 backdrop-blur-sm p-4 rounded-2xl grid-cols-4 gap-4">
              {categoryTabsData.map((category) => (
                <TabsTrigger
                  key={category.id}
                  value={category.id}
                  className="flex flex-col items-center gap-3 p-4 data-[state=active]:bg-primary/15 data-[state=active]:text-primary data-[state=active]:shadow-md rounded-xl transition-all h-auto"
                >
            ...
            {categoryTabsData.map((category) => (
              ...
              >
                <div className="flex flex-wrap gap-1.5 md:gap-3 justify-center pb-4">
                  {services.map((service) => (
                    <Button
                      key={service.name}
                      onClick={() => handleServiceClick(service.name, service.description)}
                      variant="outline"
                      className="rounded-full px-2.5 py-1 md:px-5 md:py-2 h-auto text-[11px] md:text-sm bg-background/50 backdrop-blur-sm hover:bg-primary/10 hover:text-primary hover:border-primary transition-all"
                    >
                      {service.name}
                    </Button>
                  ))}
