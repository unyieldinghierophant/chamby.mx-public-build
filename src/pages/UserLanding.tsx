...
const UserLanding = () => {
  ...
  // For authenticated users, wait for roles too
  if (user && roleLoading) {
    return <FullPageSkeleton />;
  }
  return <div className="min-h-screen bg-gradient-subtle">
      {/* Simple Header matching home page */}
       <header className="fixed top-0 left-0 right-0 bg-background border-b border-border z-50">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-0 flex items-center justify-between">
          ...
          {isGuest ? (
            /* Guest header: auth links */
            <div className="flex items-center gap-4">
              <Link to="/login" className="text-foreground hover:text-primary transition-colors font-medium text-sm md:text-base">
                Iniciar sesión
              </Link>
              <Link to="/login?mode=signup" className="px-4 py-2 rounded-full bg-primary text-primary-foreground hover:bg-primary/90 transition-colors font-medium text-sm md:text-base">
                Registrarse
              </Link>
            </div>
          ) : (
      ...
      {/* Active Job Banners - only for authenticated users */}
      {!isGuest && (
      ...
      <section className={`relative min-h-[70vh] md:min-h-[80vh] lg:min-h-[90vh] flex items-center justify-center ${isGuest ? 'pt-16' : 'pt-8'} pb-12 overflow-hidden`}>
      ...
      <main className="container mx-auto px-4 pb-24">
        ...
        {/* Authenticated-only: Recent Activity */}
        {!isGuest && (
        ...
        {/* Authenticated-only: Become Provider CTA */}
        {!isGuest && role === "client" && (
          <Card className="bg-gradient-to-br from-primary/10 to-accent/10 border-primary/20 mb-4">
        ...
        {/* Guest-only: How It Works + Reviews + Trust sections */}
        {isGuest && (
          <>
            <HowItWorks />
            <ReviewsCarousel />
