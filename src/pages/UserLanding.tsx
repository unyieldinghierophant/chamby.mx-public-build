...
const UserLanding = () => {
  ...
  // Show skeleton only while auth is loading (brief flash)
  if (authLoading) {
  ...
  return <div className="min-h-screen bg-gradient-subtle">
      ...
      <header className="fixed top-0 left-0 right-0 bg-background border-b border-border z-50">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-0 flex items-center justify-between">
          ...
          {isGuest ? (
            /* Guest header — login/signup links */
            <div className="flex items-center gap-3">
              <Button variant="ghost" onClick={() => navigate(ROUTES.LOGIN)} className="text-sm font-medium">
                Iniciar sesión
              </Button>
              <Button onClick={() => navigate(ROUTES.LOGIN + '?mode=signup')} className="bg-gradient-button text-primary-foreground text-sm">
                Registrarse
              </Button>
            </div>
          ) : (
            /* Authenticated header — profile menu (desktop + mobile) */
            <>
      ...
      {/* Active Job Banner - only for authenticated users */}
      {!isGuest && (
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-5xl mx-auto text-center space-y-8">
            <Badge className="bg-white/10 text-white border-white/20 text-sm font-medium px-4 py-2 inline-flex items-center gap-2 backdrop-blur-sm">
              {isGuest ? '🔍 Encuentra al profesional ideal' : `👋 ¡Hola${profile?.full_name ? `, ${profile.full_name.split(' ')[0]}` : ''}!`}
            </Badge>
      ...
      <main className="container mx-auto px-4 pb-24">
        ...
        {/* Become Provider CTA */}
        {(isGuest || role === "client") && <Card className="bg-gradient-to-br from-primary/10 to-purple-500/10 border-primary/20 mb-4">
